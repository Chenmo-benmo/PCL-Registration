...
namespace CCLib
{
	...
	public:
		...
		**/
		void addPoint(const CCVector3 &P)
		{
			...
			//检查是否存在NaN值(not a number)，NaN == NaN和NaN != NaN都返回false Chenmobenmo 240927
			if (	P.x != P.x
				||	P.y != P.y
				||	P.z != P.z)
			{
				//replace NaN point by (0, 0, 0)
				//将存在NaN值的坐标改为(0,0,0) Chenmobenmo 240927
				CCVector3 fakeP(0, 0, 0);
				m_points.push_back(fakeP);
			}
			else
			{
				//将坐标添加到vector中 Chenmobenmo 240927
				m_points.push_back(P);
			}

			//设置不可用 Chenmobenmo 240927
	...
	protected:
		...
		**/
		...
		//添加删除函数 Chenmobenmo 241024
		inline void erasePoint(unsigned index) { assert(index < size()); m_points.erase(m_points.begin() + index); }
