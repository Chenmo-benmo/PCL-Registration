...
//Ìí¼ÓÍø¸ñ Chenmobenmo 241023
template <class Type> class ChenmoTuple3
{
public:
	Type x, y, z;
public:

	inline ChenmoTuple3() : x(0), y(0), z(0) {}
	inline ChenmoTuple3(Type x, Type y, Type z) : x(x), y(y), z(z) {}
	inline ChenmoTuple3(const ChenmoTuple3& other) : x(other.x), y(other.y), z(other.z) {}

	inline bool operator == (const ChenmoTuple3& other) const { return this->x == other.x && this->y == other.y && this->z == other.z; }
	inline bool operator () (const ChenmoTuple3& Left, const ChenmoTuple3& Right) const { return Left == Right; }
	inline ChenmoTuple3& operator = (const ChenmoTuple3& other) const { return ChenmoTuple3(other); }

	struct Hash
	{
		std::size_t operator()(const ChenmoTuple3& t) const {
			const std::size_t prime1 = 31;
			const std::size_t prime2 = 37;
			const std::size_t prime3 = 43;

			std::size_t h1 = std::hash<int>()(t.x) * prime1;
			std::size_t h2 = std::hash<int>()(t.y) * prime2;
			std::size_t h3 = std::hash<int>()(t.z) * prime3;
			// Combine the hashes of the individual components  
			// (this is a simple example, there are better ways to combine
			return (h1 ^ (h2 << 5)) + h3;
		}
	};
};
