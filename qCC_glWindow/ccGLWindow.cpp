...
void ccGLWindow::resizeGL(int w, int h)
{
	...
	displayNewMessage(	QString(""/*"New size = %1 * %2 (px)").arg(m_glViewport.width()).arg(m_glViewport.height() //注释 Chenmobenmo 241106*/),
...
void ccGLWindow::drawForeground(CC_DRAW_CONTEXT& CONTEXT, RenderingParams& renderingParams)
{
	...
	if (m_displayOverlayEntities)
	{
		...
		if (!m_captureMode.enabled)
		{
			...
			//hot-zone
			{
				//drawClickableItems(0, yStart); //注释热区 Chenmobenmo 241106
			}
...
void ccGLWindow::setView(double rad, bool forceRedraw/*=true*/)
{
	bool wasViewerBased = !m_viewportParams.objectCenteredView;
	if (wasViewerBased)
		setPerspectiveState(m_viewportParams.perspectiveView, true);
	...
	//标记 Chenmobenmo 240920
	m_viewportParams.viewMat = ccGLUtils::GenerateViewMat(rad);
