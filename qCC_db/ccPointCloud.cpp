...
bool ccPointCloud::reserve(unsigned newNumberOfPoints)
{
	...
	//预留附加数据的空间 Chenmobenmo 240926
	if (newNumberOfPoints < size())
		return false;

	//call parent method first (for points + scalar fields)
	if (	!BaseClass::reserve(newNumberOfPoints)		//预留点云数据空间	Chenmobenmo 240926
		||	(hasColors() && !reserveTheRGBTable())		//预留颜色数据空间	Chenmobenmo 240926
		||	(hasNormals() && !reserveTheNormsTable())	//预留法向量数据空间	Chenmobenmo 240926
		||	(hasFWF() && !reserveTheFWFTable()))		//预留FWF数据空间		Chenmobenmo 240926
	{
	...
	//double check
	//再次检查空间是否分配成功		Chenmobenmo 240926
...
//添加滤波器 Chenmobenmo 241020
void ccPointCloud::filter(float probability/*删除周围点数百分比*/, float gridLength/*网格大小*/)
{
	struct EP
	{
		float x;
	...
	std::vector<EP> Point;
	Point.reserve(count);
	{// 因为数据巨大，在后面的循环里大量使用指针 point(i)->xyz 貌似更慢
		for (unsigned i = 0; i < count; i++)
			Point.push_back({ point(i)->x, point(i)->y, point(i)->z, i });
	}

	//float probability = 0.05; //删除周围点数最少的 5% 的点

	std::vector<unsigned> LowHPPoints;
	LowHPPoints.reserve((unsigned)(count * probability));
	{
		struct EDHP
		{
			unsigned HP; //一个矩阵内其他点的个数
			unsigned count;
		};

		std::vector<EDHP> PointHPData;
		PointHPData.reserve(count);
		{
			std::unordered_map<ChenmoTuple3<int>, std::vector<unsigned>, ChenmoTuple3<int>::Hash> grid; //声明网格
			//float gridLength = 0.5; //网格大小

			for (unsigned i = 0; i < count; i++) //遍历
			{
				float x = Point.at(i).x; //获取当前点的x坐标
				float y = Point.at(i).y; //获取当前点的y坐标
				float z = Point.at(i).z; //获取当前点的z坐标

				//设置网格大小
				//因为网格是虚拟的，网格位置并不需要和真实位置一致
				//相当于网格大小永远是 1，但缩放整体图像 1 / gridLength 倍，得到和设置网格大小一样的效果
				int gx = static_cast<int>(std::floor(x / gridLength)); //计算网格单元格的x索引（向下取整）
				int gy = static_cast<int>(std::floor(y / gridLength)); //计算网格单元格的y索引（向下取整）
				int gz = static_cast<int>(std::floor(z / gridLength)); //计算网格单元格的z索引（向下取整）
				grid[ChenmoTuple3<int>(gx, gy, gz)].push_back(i); //将当前点的索引添加到对应的网格单元格中
			}

			for (unsigned i = 0; i < count; i++)
			{
				float x = Point.at(i).x;
				float y = Point.at(i).y;
				float z = Point.at(i).z;

				//获取该点所在网格位置
				int gx = static_cast<int>(std::floor(x / gridLength));
				int gy = static_cast<int>(std::floor(y / gridLength));
				int gz = static_cast<int>(std::floor(z / gridLength));
				...
				for (int dx = -1; dx <= 1; dx++)
					for (int dy = -1; dy <= 1; dy++)
						...
						{ //遍历当前点所在网格以及周围网格
							auto it = grid.find(ChenmoTuple3<int>(gx + dx, gy + dy, gz + dz));
							if (it != grid.end())
							{
								for (unsigned index : it->second)
								{
									float px = Point.at(index).x;
									float py = Point.at(index).y;
									float pz = Point.at(index).z;

									//探求的距离差和一个网格大小一致，利用率最大化
									if (std::abs(px - x) <= gridLength && std::abs(py - y) <= gridLength && std::abs(pz - z) <= gridLength)
										if (index != i)
											HP++;
								}
							}
				...
				PointHPData.push_back({ HP, i });
			}
		}

		std::sort(PointHPData.begin(), PointHPData.end(), [](const EDHP& a, const EDHP& b) { return a.HP < b.HP; });

		for (unsigned i = 0; i < (unsigned)(count * probability); i++)
			LowHPPoints.emplace_back(PointHPData.at(i).count);
	}

	//因为使用 erase 所以先使索引从大到小排序
	std::sort(LowHPPoints.begin(), LowHPPoints.end(), std::greater<unsigned>());
	for (unsigned i : LowHPPoints)
		erasePoint(i);
}

//平移 Chenmobenmo 240928
void ccPointCloud::translate(const CCVector3& T)
{
	//检查平移距离，过小则没有必要平移 Chenmobenmo 240928
	if (fabs(T.x) + fabs(T.y) + fabs(T.z) < ZERO_TOLERANCE)
		return;

	//+=重载，对每个点进行平移T.x、T.y、T.z的距离 Chenmobenmo 240928
	unsigned count = size();
	{
		for (unsigned i = 0; i < count; i++)
			*point(i) += T;
	...
	//释放与点云数据相关的顶点缓冲区对象 Chenmobenmo 240929
	notifyGeometryUpdate(); //calls releaseVBOs()
...
//添加旋转 Chenmobenmo 240929
void ccPointCloud::revolve(int rad, std::string name)
{
	Get_barycenter_point(rad, name);//添加 Chenmobenmo 241001 //注释 Chenmobenmo 241002 //需要修改1 Chenmobenmo 241002

	CCVector3 T(0, 0, 0);
	double radians = (float)rad * (M_PI / 180.0);

	unsigned count = size();
	{
		for (unsigned i = 0; i < count; i++)
		{
			float newX = point(i)->x * cos(radians) - point(i)->z * sin(radians);
			float newZ = point(i)->x * sin(radians) + point(i)->z * cos(radians);
...
//添加获取中心点坐标 Chenmobenmo 241001
void ccPointCloud::Get_barycenter_point(int rad, std::string name)
{
	/*让点云与底面构成一个立体，求出重心*/
	unsigned count = size();
	float centerX = 0;
	...
	for (unsigned i = 0; i < count; i++)
	{
		...
		if (min_x >= point(i)->x) min_x = point(i)->x;
		if (max_x <= point(i)->x) max_x = point(i)->x;
	}
	float NewX = centerX / (1.0 * (float)count);
	float NewY = centerY / (1.0 * (float)count);
	float NewZ = centerZ / (2.0 * (float)count);

	//记录
	float vital_Z = NewZ;
	ccVitalZ::Set_Vital_Z(vital_Z, name);
	ccVitalZ::Set_BaryXYZ(NewX, NewY, NewZ, name);
	ccVitalZ::Set_MMX(min_x, max_x, name);
	ccVitalZ::Set_High(NewZ, name);

	//旋转
	double radians = (float)rad * (M_PI / 180.0);
	float newX = NewX * cos(radians) - NewZ * sin(radians);
	float newY = NewY;
	float newZ = NewX * sin(radians) + NewZ * cos(radians);

	barycenter_point_x = newX;
	barycenter_point_y = newY;
	barycenter_point_z = newZ;
}

//获取点云中最大最小X、Y、Z坐标 Chenmobenmo 241003
void ccPointCloud::Get_MM_XZ()
{
	min_x = point(0)->x;
	max_x = point(0)->x;
	min_y = point(0)->y;
	...
	for (unsigned i = 0; i < count; i++)
	{
		if (min_x >= point(i)->x) min_x = point(i)->x;
		if (max_x <= point(i)->x) max_x = point(i)->x;
		if (min_y >= point(i)->y) min_y = point(i)->y;
		if (max_y <= point(i)->y) max_y = point(i)->y;
		if (min_z >= point(i)->z) min_z = point(i)->z;
		if (max_z <= point(i)->z) max_z = point(i)->z;
	}
}
